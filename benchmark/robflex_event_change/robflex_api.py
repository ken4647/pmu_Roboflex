...
def robflex_apply_event(event_name: str, timeout: int = 0, strength: int = 1) -> int:
    """
    通过守护进程在共享内存中置位事件（对应 C: robflex_apply_event）。
    """
    payload = {
        "cmd": "apply_event",
...
def robflex_cancel_event(event_name: str, strength: int = 1) -> int:
    """
    通过守护进程清除事件位（对应 C: robflex_cancel_event）。
    """
    payload = {
        "cmd": "cancel_event",
        "event_name": event_name,
